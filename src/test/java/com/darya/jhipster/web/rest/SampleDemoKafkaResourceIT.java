...
import com.darya.jhipster.config.KafkaProperties;
...
class SampleDemoKafkaResourceIT {
    ...
    void setup() {
        ...
        SampleDemoKafkaResource kafkaResource = new SampleDemoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sample-demo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sample-demo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
